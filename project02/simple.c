...
char *getTimeStr(double time) {
    ...
    ms = time - (min * 60000) - (sec * 1000);

    if (time >= 60000) {
        snprintf(buf, 49, "%d min %d sec %.1f ms", min, sec, ms);
    } else if (time >= 1000) {
        snprintf(buf, 49, "%d sec %.1f ms", sec, ms);
    } else {
        snprintf(buf, 49, "%.1f ms", ms);
    }
...
cl_kernel setupKernel(const char *kernel_source, char *kernel_name, int num_args, ...) {
    ...
    for (i = 0; (i < num_args) && (kernel != NULL); i++) {
        ...
        switch (kernel_args[i].arg_t) {
            SETUP_ARG(Double, double)
            SETUP_ARG(Float, float)
            SETUP_ARG(Int, int)
            SETUP_ARG(Bool, bool)
            case IntConst:
...
cl_int launchKernel(cl_kernel kernel, int dim, size_t *global, size_t *local) {
    ...
    clock_gettime(CLOCK_REALTIME, &start);
    if (CL_SUCCESS != clEnqueueNDRangeKernel(commands, kernel, dim, NULL, global, local, 0, NULL, NULL)) {
        if (!verbose) {
...
cl_int freeDevice() {
    cl_int err;
    for (int i = 0; i < num_kernel_args; i++) {
        if ((kernel_args[i].arg_t == FloatArr) || (kernel_args[i].arg_t == DoubleArr)) {
